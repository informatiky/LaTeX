...
In my endless search for a fitting topic to write my Maths Extended Essay in, I came accross the branch of combinatorics. Never having understood the underlying field of study, and knowing that we would cover very little of it in the AI HL syllabus, I was interested in exploring it further.

To briefly help the reader to understand this essay better, below are a few definitions which might come in handy throughout the next few pages:
An Objects is a 

To understand the rest of this essay, it is important to clarify the concept of what are coloquially known as \textit{combinations}. Imagine a simple cake in which all ingredients are mixed independent of the order. In mathematical language, that is called a combination of objects. If, on the other hand, the order matters, like for a phone password, that is called a permutation. There is a further distinction between a combination and permutation where repeats are allowed or not. A phone password, for example, can have repeating digits, arranging people's names on a list, on the other hand, cannot.

Calculating possible arrangements of objects depends on the type 
...
\begin{itemize}
    \item A \underline{node} is the same as a vertex and is the undivisble point at which one or more edges meet.\par
    \item An \underline{edge} is the connection between nodes.\par
    \item The \underline{degree} of a node is the number of connections, i.e. edges, it has to another node.  \par
\end{itemize}
